...
- [恋するへびべあ診断（日文主站）](https://kumatype-shindan.xyz)：通过 20 个问题，在约 1-2 分钟内得到 16 种熊系恋爱类型之一。
- [くまタイプ診断 MBTI 16タイプ一覧（日文）](https://kumatype-shindan.xyz/types)：查看各类型名称、MBTI 风格代码、恋爱倾向和相性阅读方式。
- [くまタイプ相性一覧（日文）](https://kumatype-shindan.xyz/compatibility)：选择自己和对方的 Kuma 类型，比较相性分数和解释。
- [ラブタイプ診断64（日文）](https://kumatype-shindan.xyz/lovecharacter64)：最近新增的 64 类型恋爱角色诊断，从恋爱距离感、撒娇方式、现实感、自由度和未来倾向等维度生成结果。
- [ラブタイプ診断64の結果一覧（日文）](https://kumatype-shindan.xyz/lovecharacter64/results)：查看全部 64 个结果图片、类型代码和简短恋爱倾向说明。
- [3D KUMA（日文）](https://kumatype-shindan.xyz/3d-bears)：用毛绒玩具风和黏土手办风 3D 图片查看 16 个类型。

## 更新记录

- 2026-07：新增 Love Character 64，提供 64 类型恋爱角色诊断和结果列表。
- 2026-07：补充 Love Character 64 结果页和分享图片能力。
- 2026-07：将 README 调整为日文主版本，并新增英文版和简体中文版。
- 2026-07：将 README 首页预览图替换为没有语言选择弹窗的干净首屏截图。

## 数据和工具目录

| 资源 | 用途 | 关键内容 | 路径 |
| --- | --- | --- | --- |
| 问题数据 | 诊断模型检查 | 20 个公开问题、轴和 positive pole 元数据 | `data/questions.json` |
| Kuma 类型 | 结果内容检查 | 16 个类型的名称、slug、描述、特征、分组和相性链接 | `data/kuma-types.json` |
| 素材 manifest | 公开素材审查 | 从站点项目带出的图片和字体来源元数据 | `data/asset-manifest.json` |
| 计分 helper | 结果复现 | 用于结果和相性计算的公开 JavaScript helper | `src/scoring.mjs` |
| 计分文档 | 人类可读规则 | 轴计分、平局行为、相性计分和标签 | `docs/scoring.md` |
| 公开边界文档 | public/private 分离 | 本仓库有意包含和排除的内容 | `docs/public-boundary.md` |
| 公开检查 | 回归保护 | 针对计分和仓库边界的 Node 测试 | `tests/` |
...
1. 查看 `data/` 目录中的数据文件，尤其是 `data/questions.json` 和 `data/kuma-types.json`。
2. 阅读 `docs/scoring.md`，或 import `src/scoring.mjs` 来复现结果行为。
3. 在提出数据或计分变更前运行公开检查：
