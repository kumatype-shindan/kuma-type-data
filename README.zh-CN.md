...
- 恋するへびべあ診断：通过 20 个问题，在约 1-2 分钟内得到 16 种熊系恋爱类型之一。
- 16 类型列表：查看各类型名称、MBTI 风格代码、恋爱倾向和相性阅读方式。
- 相性诊断：选择自己和对方的 Kuma 类型，比较相性分数和解释。
- Love Character 64：最近新增的 64 类型恋爱角色诊断，从恋爱距离感、撒娇方式、现实感、自由度和未来倾向等维度生成结果。
- Love Character 64 结果列表：查看全部 64 个结果图片、类型代码和简短恋爱倾向说明。
- 3D KUMA：用毛绒玩具风和黏土手办风 3D 图片查看 16 个类型。
